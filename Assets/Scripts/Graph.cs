...
public class Graph : MonoBehaviour
{
    ...
    List<Node> GetNeighbors(int x, int y, Node[,] nodeArray, Vector2[] direction)
    {
    ...
    public float GetNodeDistance(Node source, Node target)
    {
        ...
        int stragihtSteps = max - min;

        return (1.4f * diagonalSteps + stragihtSteps);
    }

    public int GetManhatanDistance(Node source, Node target)
    {
        int dx = Mathf.Abs(source.xIndex - target.xIndex);
        int dy = Mathf.Abs(source.yIndex - target.yIndex);
        return (dx + dy);
